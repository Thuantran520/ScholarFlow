...
// ---------------------------------------------------------------------------
const GOAL_OPTIONS = ["research", "thesis", "paper", "review"];
const GOAL_I18N_MAP = { research: "goal_research", thesis: "goal_thesis", paper: "goal_paper", review: "goal_review" };
function populateGoalSelect() {
  const sel = document.getElementById("f-goal");
  if (!sel || sel.options.length > 0) return;
  const noneOpt = document.createElement("option");
  noneOpt.value = "";
  noneOpt.textContent = getI18nText("goal_none") || "— Chọn mục đích —";
  sel.appendChild(noneOpt);
  GOAL_OPTIONS.forEach(g => {
    const opt = document.createElement("option");
    opt.value = g;
    opt.textContent = getI18nText(GOAL_I18N_MAP[g]) || g;
    sel.appendChild(opt);
  });
}

function goalLabelText(goalVal) {
  const key = GOAL_I18N_MAP[goalVal];
  if (!key) return goalVal || "";
  return getI18nText(key) || goalVal;
}
...
function saveCurrentToBiblio() {
  ...
  const goalVal = (document.getElementById("f-goal")?.value || "").trim();
  ...
  const newItem = {
    ...
    meta: { ...currentMeta, notes: notesVal, tag: tagVal, goal: goalVal },
    tag: tagVal,
    notes: notesVal,
    goal: goalVal,
    savedAt: Date.now()
  ...
  storSet({ saved_bibliographies: savedBibliographies }, () => {
    updateBiblioBadges();
    maybeCreateLinkedTodo(newItem, goalVal);
  });
}

function maybeCreateLinkedTodo(item, goalVal) {
  if (!goalVal) return;
  storGet("sf_todos", (res) => {
    let todos = (res && Array.isArray(res.sf_todos)) ? [...res.sf_todos] : [];
    const todoUrl = item.meta?.url || "";
    const existingIdx = todos.findIndex(t => t.libId === item.id || (todoUrl && t.detail === todoUrl));
    const goalTitle = "🎯 " + goalLabelText(goalVal) + ": " + (item.meta?.title || item.meta?.url || "");
    if (existingIdx !== -1) {
      todos[existingIdx].text = goalTitle;
      todos[existingIdx].detail = item.meta?.url || "";
      todos[existingIdx].goal = goalVal;
      todos[existingIdx].libId = item.id;
    ...
    todos.unshift({
      ...
      text: "🎯 " + goalLabelText(goalVal) + ": " + (item.meta?.title || item.meta?.url || ""),
      detail: item.meta?.url || "",
      priority: "normal",
      done: false,
      createdAt: timeStr,
      libId: item.id,
    ...
    storSet({ sf_todos: todos });
    window.dispatchEvent(new CustomEvent("sf:todos-changed"));
    showToast("todo_toast_linked", "success");
  });
}

function openSourceFromLibId(libId) {
  currentModalFilter = "";
  const searchInput = document.getElementById("biblio-search-input");
  if (searchInput) searchInput.value = "";
  const bm = document.getElementById("biblio-modal");
  if (bm) bm.style.display = "block";
  renderBiblioModalList();
  const card = document.getElementById("bib-" + libId);
  if (card) {
  ...
  filtered.forEach((item, index) => {
    ...
    actions.appendChild(btnCopy);
      ...
      btnEdit.addEventListener("click", () => {
        ...
        currentMeta.goal = item.goal || item.meta?.goal || "";
        ...
        if (goalInput) goalInput.value = currentMeta.goal;
        syncInputs();
        updateCitationDisplay();
