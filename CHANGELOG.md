...
- **Media player: video THƯỜNG (không live) bị nhận diện nhầm là live → thanh đỏ full, MẤT dấu tròn kéo, không seek/prev được**: YouTube giữ sẵn một `.ytp-live-badge` trong control bar của **mọi** video và bật/tắt bằng thuộc tính `hidden`, nhưng `querySelector(".ytp-live-badge")` vẫn match cả phần tử đang ẩn → video VOD bị gắn cờ live. Nay chỉ tính badge **hiển thị** (`.ytp-live-badge:not([hidden])`), khớp đúng cách `main.js` vẫn làm. Kèm **dọn 2 cảnh báo "Lỗi bản đồ nguồn: NetworkError"** khi bật DevTools trên Firefox (`peer.min.js`/`qrious.min.js` trỏ tới file `.map` không đóng gói): gỡ comment `sourceMappingURL` khỏi 2 lib (giữ nguyên code, vẫn `node --check` OK).
