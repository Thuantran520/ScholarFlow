...
- **Media player: nhận diện live cho MỌI nền tảng (không chỉ YouTube)** — thêm bộ dò **platform-agnostic**: trên livestream, **cạnh trái của `video.seekable` (cửa sổ DVR) tự trượt tới theo thời gian thực** (không thể tua lùi quá cửa sổ lưu), còn video thường (VOD) thì cạnh trái **đóng đinh ở 0** mãi mãi. Đếm độ trượt này qua các lần poll (≥5s) ⇒ suy ra live **không cần hack DOM theo từng trang** — bắt được cả live có **độ-dài-hữu-hạn** mà trước đây lọt lưới (Twitch, Facebook/Instagram Live, HLS.js/dash.js generic, Kick…). Tín hiệu `duration` vô hạn/0 + `seekable` vô hạn vẫn giữ cho mọi trang ngoài YouTube; phần DOM (`.ytp-live`, chip render, `/live/`) giờ chỉ là **tín hiệu phụ riêng cho YouTube**. Có seam `__sfMedia._liveEdge` + test: cạnh trượt → live, cạnh đứng yên → không live.
