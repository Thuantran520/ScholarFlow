...
- **Media player: video YouTube THƯỜNG (không live) vẫn bị nhận diện là live → thanh đỏ full, MẤT dấu tròn kéo, không seek/prev được**: thủ phạm gốc là **`getStartDate()`** — mình giả định video VOD trả epoch 1970, nhưng YouTube phát VOD qua MSE nên `getStartDate()` trả **một mốc thời gian THẬT**, khiến `t > 1000 → isLive = true` gán live cho **mọi** video YouTube. Nay `getStartDate()` **chỉ điền đồng hồ elapsed** (`liveStart`) khi video **đã** được kết luận live bằng tín hiệu bền (duration vô hạn/0, `seekable` vô hạn, hoặc marker của **chính player** đó), chứ **không bao giờ tự quyết định live** nữa. Đồng thời **marker live được scope vào đúng shell `.html5-video-player` của video đang phát** (chỉ nhận `.ytp-live`/badge **ẩn bằng `[hidden]`** trong shell đó): badge của một player teaser/ambient/"breaking news" khác trên trang **không còn rò rỉ** sang video ta đang xem — tức "bắt đúng video". Kèm **dọn 2 cảnh báo "Lỗi bản đồ nguồn: NetworkError"** khi bật DevTools trên Firefox (`peer.min.js`/`qrious.min.js` trỏ `.map` không đóng gói): gỡ comment `sourceMappingURL` (giữ nguyên code, vẫn `node --check` OK).
