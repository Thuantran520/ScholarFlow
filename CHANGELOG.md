...
- **Media player: video YouTube THƯỜNG (không live) vẫn bị nhận diện là live → thanh đỏ full, MẤT dấu tròn kéo, không seek/prev được**: hai giả định sai cộng lại. (1) **`getStartDate()`** — mình tưởng VOD trả epoch 1970, nhưng YouTube phát VOD qua MSE nên nó trả **mốc thời gian THẬT**, khiến `t > 1000 → isLive = true` gán live cho **mọi** video YouTube. Nay `getStartDate()` **chỉ điền đồng hồ elapsed** (`liveStart`) khi video **đã** được kết luận live bằng tín hiệu bền, **không bao giờ tự quyết định live** nữa. (2) **`.ytp-live-badge`** — đã xác minh: node này **luôn có** trong control bar của MỌI video YouTube và được bật/tắt bằng thuộc tính **`disabled`** (video thường = badge `disabled`/ẩn CSS), KHÔNG phải `hidden`. Code cũ `:not([hidden])` vẫn khớp badge thường → báo live giả. Nay chỉ tính badge **không có cả `[disabled]` lẫn `[hidden]`**. Toàn bộ marker (badge + class `.ytp-live` của root player) được **scope vào đúng shell `.html5-video-player` của video đang phát**, nên badge của một teaser/ambient player khác **không rò rỉ** sang video ta xem ("bắt đúng video"). Và vì YouTube có nhiều `<video>` phụ (preloader/mini-player/Shorts) báo `duration` Infinity/NaN cho cả video thường, **trong ngữ cảnh YouTube mình TẮT hoàn toàn heuristic "duration không hữu hạn = live"** — chỉ tin cờ live của chính player (`.ytp-live`/badge enabled) hoặc URL `/live/`; heuristic duration vẫn giữ cho player HTML5/MSE thường ở trang khác. Kèm **dọn 2 cảnh báo "Lỗi bản đồ nguồn: NetworkError"** trên Firefox khi bật DevTools (`peer.min.js`/`qrious.min.js` trỏ `.map` không đóng gói → gỡ comment `sourceMappingURL`, giữ nguyên code, vẫn `node --check` OK).
