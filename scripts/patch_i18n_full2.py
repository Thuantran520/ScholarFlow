...
# I will replace the English blocks that were injected for ZH, RU, JA.
# Actually, the easiest way is to just append them to the very end of the language blocks!
# Wait, if there are duplicate keys, the LAST one in the JSON-like object wins!
# So if I inject right before the closing brace `}` of each language, it will OVERRIDE everything!
...
def inject_bottom(code, lang_id, new_dict):
    # Find the block for the language
    # Assuming standard structure: `"vi": { ... },`
    pattern = rf'("{lang_id}":\s*{{(?:[^{{}}]*|\{{[^{{}}]*\}})*?)(\s*}})'
    ...
    if match:
        ...
        block = block.rstrip(',')
        return code[:match.start()] + block + match.group(2) + code[match.end():]
    return code

code = inject_bottom(code, 'vi', vi_new)
code = inject_bottom(code, 'en', en_new)
code = inject_bottom(code, 'zh', zh_new)
code = inject_bottom(code, 'ru', ru_new)
code = inject_bottom(code, 'ja', ja_new)
